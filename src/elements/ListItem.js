...
const ListItem = (modalVisible, setModalVisible, setItemId, setDetailTitle,setDetailDate,setDetailImage,setDetailDimensions,setDetailArtist,setDetailDesc, setArtistId) => {
    ...
    const Item = ({ title, id, image_id, dimensions,artist_title,date_start,date_end,description, artist_id}) => {
      ...
      if (imageId !=null) {
        return (
          <TouchableHighlight onPress={()=> {setModalVisible(!modalVisible)}}  onPressIn={()=> {{setItemId(id);setDetailTitle(title);setDetailDate(getDateRange());setDetailImage(image_id);setDetailDimensions(dimensions);setDetailArtist(artist_title); setDetailDesc(description); setArtistId(artist_id)}}} underlayColor='lightgray'>
            <View>
        ...
        return (
          <TouchableHighlight onPress={()=> {setModalVisible(!modalVisible)}}  onPressIn={()=> {{setItemId(id);setDetailTitle(title);setDetailDate(getDateRange());setDetailImage(null);setDetailDimensions(dimensions);setDetailArtist(artist_title); setDetailDesc(description); setArtistId(artist_id)}}} underlayColor='lightgray'>
            <View>
              <View style={styles.listelement}>
